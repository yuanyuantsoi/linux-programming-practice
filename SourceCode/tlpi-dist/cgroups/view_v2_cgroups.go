...
--files         Comma-separated list of filenames to display (if present).
`)
...
type cmdLineOptions struct {
	...
	files               []string // Files to display per cgroup (if present)
}
...
func parseCmdLineOptions() cmdLineOptions {
	...
	filesPtr := flag.String("files", "",
		"Comma-separated list of cgroup files to display (if present)")
	...
	if strings.TrimSpace(*filesPtr) != "" {
		for _, f := range strings.Split(*filesPtr, ",") {
...
const BOLD = ESC + "[1m"
const LIGHT_BLUE = ESC + "[96m"
const BRIGHT_GREEN = ESC + "[92m"
const BLUE = ESC + "[34m"
const MAGENTA = ESC + "[35m"
const LIGHT_PURPLE = ESC + "[95m"
...
var cgroupColor = map[string]string{
	"root":            "",
	"domain":          "",
	"domain threaded": UNDERLINE + BOLD + BRIGHT_GREEN,
	"threaded":        BRIGHT_GREEN,
...
func displayCgroup(cgroupPath string, rootSlashCnt int,
	...
	fmt.Print(indent)
	if opts.useColor {
		fmt.Print(cgroupColor[cgroupType])
	}
	fmt.Print(displayPath)
	if opts.useColor {
		fmt.Print(NORMAL)
	}
	fmt.Print(" " + cgroupAbbrev[cgroupType])
	...
	if len(opts.files) > 0 {
		err = displayCgroupFiles(cgroupPath, indent+"    ", opts)
		if err != nil {
			return err
		}
	}
...
// Display the named files (if present) for the cgroup specified by 'cgroupPath'.

func displayCgroupFiles(cgroupPath string, indent string, opts cmdLineOptions) error {
	...
	for _, name := range opts.files {
		// Avoid path traversal / absolute paths.
		if name == "" || strings.Contains(name, "/") || strings.Contains(name, "..") {
			continue
		}

		p := filepath.Join(cgroupPath, name)
		...
		// Collapse internal whitespace/newlines for readable wrapping.
		val = strings.Join(strings.Fields(val), " ")

		line := wrapText(val, name+": ", displayWidth, indent)

		if opts.useColor {
			line = colorEachLine(line, GRAY)
		}
...
func isRealtimeThread(tid int) (bool, error) {
	...
	ret, _, err := syscall.Syscall(syscall.SYS_SCHED_GETSCHEDULER,
		uintptr(tid), uintptr(0), uintptr(0))
...
var ansiColorSeqRE = regexp.MustCompile(ESC + "[^m]*m")

func displayLength(word string) int {
	s := ansiColorSeqRE.ReplaceAllString(word, "")
	return len(s)
}
